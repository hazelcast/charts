...
|`+mancenter.existingConfigMap+` |ConfigMap which contains Hazelcast Client configuration file(s) that are used instead hazelcast-client.yaml embedded into values.yaml |`+{DEFAULT_HAZELCAST_CLIENT_YAML}+`
