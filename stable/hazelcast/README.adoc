...
|`+mancenter.adminCredentialsSecretName+` |Kubernetes Secret Name for admin
credentials. Secret has to contain `username` and `password` literals. please check
Management Center documentation for password requirements |`+nil+`
...
Then use the same secret with the parameter `mancenter.adminCredentialsSecretName` to install hazelcast helm chart.
[source,bash]
----
$ helm install --name hazelcast --set mancenter.adminCredentialsSecretName=admincredentials hazelcast/hazelcast
=======
== Notable changes
