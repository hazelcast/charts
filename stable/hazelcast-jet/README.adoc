...
|`+customVolumeClaimName+` |Configuration for a volume claim name which will
...
You can mount any existing Persistent Volumes which contains your JAR files
to the pods created by helm chart via a Persistent Volume Claim using 
`customVolumeClaimName` configuration property.

When the `customVolumeClaimName` set, it will mount the Persistent Volume
backed by Persistent Volume Claim to the pod on `/data/custom` path.
...
# Helm 3
$ helm install my-jet-release \
  --set customVolumeClaimName=existing-pv-claim-name-which-holds-jars \
    hazelcast/hazelcast-jet
